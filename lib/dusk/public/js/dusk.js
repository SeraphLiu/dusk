var Dusk = function(target) {
  ...
  this.step   = 60000    // 10 seconds per value
  this.width  = 940      // 940 grid

  this.colors = [ "#08519c", "#*82bd", "#6baed6", "#fee6ce", "#fdae6b", "#e6550d" ]
  //this.colors = [ "#ddic77", "#c994c7","#e7eief", "#efedf5", "#bcbddc", "#756bb1" ]
  ...
  this.horizon = this.context.horizon().height(30).colors(this.colors)
  ...
  this.generateHorizons = function() {
    ...
    myGraphite.find(this.target, function(error, results) {
      ...
      for (var i=0; i<results.length; i++) {
        var metric = myGraphite.metric(results[i]).summarize("avg");
